...
temp.hw.summary <- temp.hw.only %>% 
  group_by(site_code) %>% summarize(mean_daily_temp_hw = mean(mean_temp_C, na.rm=T), mean_daily_max_temp_hw = mean(max_temp_C, na.rm=T)) %>% mutate(site_code = if_else(site_code == "BS", "WBS", site_code))
...
mort.plot <- ggplot(data = mort.model.df, aes(col = degrees_from_north, y = prop_dead*100, x = mean_daily_max_temp_hw)) + geom_jitter(size = 2, width = 0.2, alpha = 0.7) + scale_color_gradientn(colors = wes_palette("Zissou1", 20, type = "continuous"), limits = c(0,180)) + theme_classic() + labs(y = "Mortality (%)", col = "Direction relative to north (º)", x = "Mean maximum daily temperature (ºC)") + theme(axis.title = element_text(size = 14), axis.text = element_text(size = 12), legend.text = element_text(size = 12), legend.title = element_text(size = 14)) + geom_smooth(aes(y = fit*100), data= new.dat, method = "lm", se = T, col= "black") + theme(legend.direction = "horizontal", legend.position = "bottom") 
mort.plot

ggsave(filename = "../outputs/Fig3.png", dpi = 1200, mort.plot, height = 3, width = 3.5, units = "in", scale = 1.5)
