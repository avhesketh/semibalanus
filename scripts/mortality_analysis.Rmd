...
hourly.temp.vic <- read_csv("../raw_data/hourly_temps_figure/hourly_temp_vicairport.csv", col_select = c(5:8, 10, 14)) %>% filter(Day <= 30 & Day >=23) %>% rename(date_time = 1, temp = 5, humidity = 6) %>% mutate(date_time = ymd_hms(date_time)) %>% View()
...
S1Fig1 <- bamfield.panel / yyj.panel + plot_annotation(tag_levels = "A")
S1Fig1
...
survey.info <- read_csv("../raw_data/SBC_siteinformation.csv") %>% 
  select(site_code, latitude_degrees, longitude_degrees, mortality, mortality_survey, degrees_from_north)  %>% na.omit() 
...
imputed.tofino.hw <- hourly.temp.tofino %>% select(-Day, -Month,-time) %>% separate(date_time, into = c("date","time"), sep = " ", remove = F) %>% mutate(site_code = "PP") %>% filter(date <= "2021-06-29" & date >= "2021-06-25")

imputed.tofino.mdmean <- imputed.tofino.hw %>% group_by(site_code, date) %>% summarize(max_temp = max(temp)) %>% ungroup() %>% group_by(site_code) %>% summarize(mean_daily_max_temp_hw = mean(max_temp))                                                 
temp.hw.summary <- temp.hw.only %>% 
  group_by(site_code) %>% summarize(mean_daily_temp_hw = mean(mean_temp_C, na.rm=T), mean_daily_max_temp_hw = mean(max_temp_C, na.rm=T)) %>% mutate(site_code = if_else(site_code == "BS", "WBS", site_code)) #%>% filter(site_code != "PP") %>% full_join(imputed.tofino.mdmean)
...
mort.mod.1 <- glmmTMB(prop_dead ~ mean_daily_max_temp_hw + mean_low_tide_time + algal_prop_cover + degrees_from_north + (1 | site_code/transect), data = mort.model.df, family = tweedie())

plot(simulateResiduals(mort.mod.1)) # this seems to work. 

#try the beta model since this is more correct

mort.mod.2 <- glmmTMB((prop_dead*nrow(mort.model.df)+0.5)/nrow(mort.model.df) ~ mean_daily_max_temp_hw + mean_low_tide_time + algal_prop_cover + degrees_from_north + (1 | site_code/transect), data = mort.model.df, family = beta_family())
...
new.dat <- data.frame(degrees_from_north = mort.model.df$degrees_from_north, 
...
mort.plot <- ggplot(data = mort.model.df, aes(col = degrees_from_north, y = prop_dead*100, x = mean_daily_max_temp_hw)) + geom_jitter(size = 2, width = 0.2, alpha = 0.8) + scale_color_gradientn(colors = wes_palette("Zissou1", 20, type = "continuous"), limits = c(0,180)) + theme_classic() + labs(y = "Mortality (%)", col = "Aspect (º)", x = "Mean maximum daily temperature (ºC)") + theme(axis.title = element_text(size = 14), axis.text = element_text(size = 12), legend.text = element_text(size = 12), legend.title = element_text(size = 14)) + geom_smooth(aes(y = fit*100), data= new.dat, method = "lm", se = T, col= "black")
mort.plot
