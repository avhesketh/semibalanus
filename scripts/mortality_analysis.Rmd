...
ggplot(data = mort.summary, aes(x = fct_reorder(site_code, longitude_degrees, min), y = average_mort*100, col = average_maxtemp)) + geom_point(size = 5) + scale_colour_gradientn(colours = wes_palette("Zissou1", type = "continuous", 10)) + geom_point(size = 5, pch = 1, color = "black") + theme_bw() +labs(y = "Mortality (%)", x = "Site", color = "Max daily temp. (ºC) \n during heat wave") + theme(axis.title = element_text(size = 14), axis.text = element_text(size = 12), legend.text = element_text(size = 12), legend.title = element_text(size = 14))
