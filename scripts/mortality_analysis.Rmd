...
In addition, we will consider the longitude of each site as a covariate (approximating distance from the more temperate outer coast). Given that topography and aspect varied within each site, it seems unlikely that using the dominant shore aspect will serve any major purpose.
...
survey.info <- read_csv("../raw_data/SBC_siteinformation.csv") %>% 
  ...
  na.omit() %>% mutate(aspect = case_when(aspect == "N" ~ 0, aspect == "S" ~ 180, aspect == "W" ~ 270, aspect == "E" ~ 90, aspect == "NW" ~ 315, aspect == "SW" ~ 225, aspect == "NE" ~ 45, aspect == "SE" ~ 135))
...
explanatory.variables <- survey.info %>% left_join(temp.hw.summary) %>% 
  full_join(temp.summary) %>% select(-mortality, -mortality_survey, -latitude_degrees)
...
mort.mod.1 <- glmmTMB(prop_dead ~ mean_daily_max_temp_hw + longitude_degrees + algal_prop_cover + number_ANEL + (1 | site_code), data = mort.model.df, family = tweedie())
...
The residuals don't look too great, so it might be worth trying a beta approach with transformation.
...
mort.summary <- mort.model.df %>% mutate(site_code = factor(site_code)) %>% group_by(site_code, longitude_degrees) %>% summarize(average_mort = mean(prop_dead, na.rm = T), average_maxtemp = mean(mean_daily_max_temp_hw)) %>% mutate(site_code = factor(site_code))
