## Characterizing infauna in Semibalanus cariosus beds
## Amelia V Hesketh
## January 2021
...
infauna.clean <- infauna.all %>% replace(is.na(.), 0) %>% # replace NA with 0 (no individuals counted)
...
#write_csv(infauna.clean, "./clean_data/SBHW_InfaunaGradient.csv")
...
#write_csv(infauna.clean.coarse, "./clean_data/SBHW_InfaunaCoarseGradient.csv")

################################################################################

### Analysis 1: NMDS & PERMANOVA to look at between-site differences in infaunal assemblages
...
infauna.summer <- infauna.clean %>% filter(collection_number %in% 3:12) %>% left_join(site.palette)
...
# ready to analyze
infauna.gradient.mds <- metaMDS(infauna.matrix, k = 3, try = 999, autotransform = T)
...
### NMDS plot of infaunal community differences in ggplot

data.scores <- as.data.frame(scores(infauna.gradient.mds))
...
species.scores <- as.data.frame(scores(infauna.gradient.mds, "species"))
...
### Plots of infaunal community abundance & diversity
...
# Plot 2A & 2B in main text: infaunal richness and Shannon diversity across sites
...
# Plot 2C in main text: infaunal abundance across coarse taxonomic groupings
...
## Characterizing seasonal shifts in infauna at two sites (SA and PP)
