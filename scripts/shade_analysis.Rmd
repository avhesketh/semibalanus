...
temp.clean <- temp.df.clean.aug %>%  full_join(temp.df.clean.july) %>% mutate(treatment = case_when(treatment == 1 ~ "UI", treatment == 2 ~ "SI", treatment == 3 ~ "UR", treatment == 4 ~ "SR")) %>% filter(date_time < "2021-08-21 6:00:00" & date_time > "2021-04-17 15:00:00")
...
temp.sep <- temp.clean %>% separate(date_time, into = c("date","time"), sep = " ")
...
There's something odd going on here with one of the SI plots. Need to look into it a bit more.
