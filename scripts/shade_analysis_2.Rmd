...
recruitment.peak <- barnacles %>% filter(date == "2021-07-07" | date == "2021-07-08") %>% group_by(plot_number, block, treatment_code) %>% summarize(total_abund = sum(abund)) %>% mutate(trt_long = case_when(treatment_code == "SR" ~ "Shaded", treatment_code == "UR" ~ "Unshaded"))

fig5c <- ggplot(recruitment.peak, aes(x = trt_long, y = total_abund, fill = trt_long, col = trt_long)) + geom_boxplot(alpha = 0.4, outlier.shape = NA) + geom_jitter(width = 0.2, alpha = 0.3) + labs(x = "Treatment", y = "Total barnacle abundance", fill = "Treatment") + theme_classic() + scale_fill_manual(values = c("skyblue3", "tomato2")) + scale_color_manual(values = c("skyblue3", "tomato2")) + theme(axis.title = element_text(size = 14), axis.text = element_text(size = 12), legend.position = "none")
...
fig5f <- ggplot(shannon.algae, aes(x = treatment_code, y = shannon, fill = treatment_code, col = treatment_code)) + geom_boxplot(alpha = 0.4, outlier.color = NA) + geom_jitter(alpha = 0.3, width = 0.2) + scale_color_manual(values = c("skyblue4","skyblue3", "tomato4", "tomato2")) + scale_fill_manual(values = c("skyblue4","skyblue3", "tomato4", "tomato2"))+ theme_classic() + theme(axis.title = element_text(size = 14), axis.text = element_text(size = 12), legend.position = "none") + labs(x = "Treatment", y = "Shannon diversity (algae)")

library(patchwork)

# assemble figure
fig5 <- (fig5a + fig5b + fig5c) / (fig5d + fig5e + fig5f) + plot_layout(guides = "collect")
...
infauna_sa <- read_csv("../raw_data/shade/SBHW_SHADE_infauna.csv") %>% rename(plot_number = plot_id) %>%  left_join(plot.info) %>% select(-plot_number, -treatment_final_numeric, -shore_level) %>% rename(treatment = treatment_original_numeric)
